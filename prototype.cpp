/*
    A prototype pattern is used in software development when the type of objects to create is determined by 
    a prototypical instance, which is cloned to produce new objects. This pattern is used, for example, 
    when the inherent cost of creating a new object in the standard way (e.g., using the new keyword) is 
...
private:
    ...
    int m_ID;
public:
    PersonRecord(string personName, int ID) : m_personName(personName), m_ID(ID) {}
    void print() override {
